...
public class RconBan implements Listener {
    ...
    public void onMessage(VKMessageEvent e) throws ApiException, ClientException {
        String command = "/s ban ";
        String commandfull = "/service ban ";
        ...
        if (e.getMessage().getText().equals("/s ban") && moders.contains(vk) | admins.contains(vk) && !(e.getPeer() == 2000000000 + Main.getInstance().getConfig().getInt("LogChat"))) {
            CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Использование: /s ban <ник> [время] <причина>").execute();
        }
        if (e.getMessage().getText().equals("/service ban") && moders.contains(vk) | admins.contains(vk) && !(e.getPeer() == 2000000000 + Main.getInstance().getConfig().getInt("LogChat"))) {
            CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Использование: /service ban <ник> [время] <причина>").execute();
        }

            if (moders.contains(vk) | admins.contains(vk)) {
                if (e.getMessage().getText().startsWith(command) | e.getMessage().getText().startsWith(commandfull)) {
                    if (auth.contains(id)) {
...
//                                    return;
                                        ...
                                        if (time.contains(num) & totime.equals("d") | totime.equals("h") | (totime.equals("m") | (totime.equals("s") | (totime.equals("mo")))) && parts[4] != null) {
                                            if (totime.equals("d")) {
                                                ...
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Игрок '" + nick + "' был забанен на " + num + " дней по причине: '" + reason + "'!").execute();
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(2000000000 + Main.getInstance().getConfig().getInt("LogChat")).message("#Баны\n\n" + ">> Игрок Rcon(id" + e.getMessage().getFromId() + ") забанил игрока " + nick + " на " + num + " дней по причине: '" + reason + "'!").execute();
                                                getServer().getScheduler().callSyncMethod(plugin, () -> {
                                                    dispatchCommand(getConsoleSender(), "tempban " + nick + " " + time + " " + reason);
                                                    return null;
                                                });
                                            }
                                            if (totime.equals("h")) {
                                                String reason = e.getMessage().getText().substring(command.length() + nick.length() + time.length() + 2);
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Игрок '" + nick + "' был забанен на " + num + " часов по причине: '" + reason + "'!").execute();
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(2000000000 + Main.getInstance().getConfig().getInt("LogChat")).message("#Баны\n\n" + ">> Игрок Rcon(id" + e.getMessage().getFromId() + ") забанил игрока " + nick + " на " + num + " часов по причине: '" + reason + "'!").execute();
                                                getServer().getScheduler().callSyncMethod(plugin, () -> {
                                                    dispatchCommand(getConsoleSender(), "tempban " + nick + " " + time + " " + reason);
                                                    return null;
                                                });
                                            ...
                                            if (totime.equals("m")) {
                                                String reason = e.getMessage().getText().substring(command.length() + nick.length() + time.length() + 2);
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Игрок '" + nick + "' был забанен на " + num + " минут по причине: '" + reason + "'!").execute();
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(2000000000 + Main.getInstance().getConfig().getInt("LogChat")).message("#Баны\n\n" + ">> Игрок Rcon(id" + e.getMessage().getFromId() + ") забанил игрока " + nick + " на " + num + " минут по причине: '" + reason + "'!").execute();
                                                getServer().getScheduler().callSyncMethod(plugin, () -> {
                                                    dispatchCommand(getConsoleSender(), "tempban " + nick + " " + time + " " + reason);
                                                    return null;
                                                });
                                            ...
                                            if (totime.equals("s")) {
                                                String reason = e.getMessage().getText().substring(command.length() + nick.length() + time.length() + 2);
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Игрок '" + nick + "' был забанен на " + num + " секунд по причине: '" + reason + "'!").execute();
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(2000000000 + Main.getInstance().getConfig().getInt("LogChat")).message("#Баны\n\n" + ">> Игрок Rcon(id" + e.getMessage().getFromId() + ") забанил игрока " + nick + " на " + num + " секунд по причине: '" + reason + "'!").execute();
                                                getServer().getScheduler().callSyncMethod(plugin, () -> {
                                                    dispatchCommand(getConsoleSender(), "tempban " + nick + " " + time + " " + reason);
                                                    return null;
                                                });
                                            ...
                                            if (totime.equals("mo")) {
                                                String reason = e.getMessage().getText().substring(command.length() + nick.length() + time.length() + 2);
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Игрок '" + nick + "' был забанен на " + num + " месяцев по причине: '" + reason + "'!").execute();
                                                CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(2000000000 + Main.getInstance().getConfig().getInt("LogChat")).message("#Баны\n\n" + ">> Игрок Rcon(id" + e.getMessage().getFromId() + ") забанил игрока " + nick + " на " + num + " месяцев по причине: '" + reason + "'!").execute();
                                                getServer().getScheduler().callSyncMethod(plugin, () -> {
                                                    dispatchCommand(getConsoleSender(), "tempban " + nick + " " + time + " " + reason);
                                                    return null;
                                                });
                                            ...
                                            if (!(time.contains("s")) | !(time.contains("m")) | !(time.contains("h")) | !(time.contains("d")) | !(time.contains("mo"))) {
                                                if (time.length() == (num.length() + 2) | time.length() == (num.length() + 1) && (!time.contains(check))) {
                                                    ...
                                                    String reason = e.getMessage().getText().substring(command.length() + Nickname1.length() + 1);
                                                        CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Игрок '" + Nickname1 + "' был забанен навсегда по причине: '" + reason + "'!").execute();
                                                        CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(2000000000 + Main.getInstance().getConfig().getInt("LogChat")).message("#Баны\n\n" + ">> Игрок Rcon(id" + e.getMessage().getFromId() + ") забанил игрока " + Nickname1 + " навсегда по причине: '" + reason + "'!").execute();
                                                        getServer().getScheduler().callSyncMethod(plugin, () -> {
                                                            dispatchCommand(getConsoleSender(), "ban " + Nickname1 + " " + reason);
                                                            return null;
                                                        });
                                                }
                                            }
...
//                                    return;
                        ...
                        CLIENT.messages().send(ACTOR).randomId(RANDOM.nextInt()).peerId(e.getPeer()).message(">> Беседа не авторизована! (/s ban)\n" +
